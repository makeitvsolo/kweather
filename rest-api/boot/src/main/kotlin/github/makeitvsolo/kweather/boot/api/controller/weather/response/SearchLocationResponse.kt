...
import github.makeitvsolo.kweather.weather.api.service.location.error.MapSearchForLocationErrorInto
...
object SearchLocationResponse : MapSearchForLocationErrorInto<ResponseEntity<*>> {

    fun fromOk(response: SearchForLocationResponse): ResponseEntity<*> =
        ResponseEntity.status(HttpStatus.OK)
            .body(OkMessage.from(HttpStatus.OK, response))

    fun fromError(error: SearchForLocationError): ResponseEntity<*> =
        error.into(this)
