...
import github.makeitvsolo.kweather.weather.api.service.weather.error.MapFetchWeatherErrorInto
import github.makeitvsolo.kweather.weather.api.service.weather.usecase.FetchWeatherResponse

import org.springframework.http.HttpStatus
import org.springframework.http.ResponseEntity

object FetchWeatherResponse : MapFetchWeatherErrorInto<ResponseEntity<*>> {

    fun fromOk(response: FetchWeatherResponse): ResponseEntity<*> =
        ResponseEntity.status(HttpStatus.OK)
            .body(OkMessage.from(HttpStatus.OK, response))

    fun fromError(error: FetchWeatherError): ResponseEntity<*> =
        error.into(this)
